...
fun Activity.hideKeyboard() {
    ...
    //If no view currently has focus, detail a new one, just so we can grab a window token from it
    if (view == null) {
        view = View(this)
    }
...
fun Long.dateInFormat(format: String): String? {
    val dateFormat = SimpleDateFormat(format, Locale.US)
    var tgl : String? = null
    try {
        val parsedDate = Date(this)
        tgl = dateFormat.format(parsedDate)
    } catch (ignored: ParseException) {
        ignored.printStackTrace()
    }
    return tgl
...
fun Long.getFormattedTimeString(time: Long): String {
    val newFormat = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.US)
    return newFormat.format(Date(time))
}

fun Long.getFormattedTimeStringNoParam(): String {
    val newFormat = SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.US)
